...
class Config:
    ...
    def from_file(cls, config_path: Path) -> "Config":
        ...
        Args:
            config_path: Path to configuration file (-šÕ¡¤ënÑ¹)
            
        Returns:
            Config: Configuration instance (-š¤ó¹¿ó¹)
        """
        if not config_path.exists():
            logger.warning(f"Config file not found: {config_path}")
            logger.warning(f"-šÕ¡¤ëL‹dKŠ~[“: {config_path}")
            return cls.default()
    ...
    def default(cls) -> "Config":
        ...
        Returns:
            Config: Default configuration (ÇÕ©ëÈ-š)
        """
        return cls(
            camera=CameraConfig(),
            room=RoomConfig(),
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        Returns:
            Dict[str, Any]: Configuration dictionary (-šžø)
        """
        return {
    ...
    def save_to_file(self, config_path: Path) -> bool:
        ...
        Args:
            config_path: Path to save configuration (-šÝXÑ¹)
        ...
        try:
            ...
            logger.info(f"-š’ÝXW~W_: {config_path}")
            return True
            
        except Exception as e:
            logger.error(f"Failed to save config file: {e}")
            logger.error(f"-šÕ¡¤ënÝXk1W: {e}")
            return False
...
def create_sample_config() -> Config:
    ...
    Returns:
        Config: Sample configuration (µó×ë-š)
    """
    return Config(
        camera=CameraConfig(
            camera_id=0,
