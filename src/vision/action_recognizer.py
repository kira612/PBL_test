...
class PoseActionRecognizer:
    ...
    def recognize_action(self, pose: PersonPose, frame_shape: Tuple[int, int]) -> ActionResult:
        ...
        Args:
            pose: Person pose information (∫iˇ‚≈1)
            frame_shape: Frame shape (height, width) (’Ï¸‡b∂)
            
        Returns:
            ActionResult: Action recognition result (L’çXPú)
        """
...
class BBoxActionRecognizer:
    ...
    def recognize_action(self, detection: Detection, frame_shape: Tuple[int, int]) -> ActionResult:
        ...
        Args:
            detection: Person detection result (∫i˙Pú)
            frame_shape: Frame shape (height, width) (’Ï¸‡b∂)
...
class ActionRecognizer:
    ...
    def __init__(self, use_pose: bool = True):
        ...
        Args:
            use_pose: Whether to use pose-based recognition (ˇ‚Ÿ¸πçXí(YãK)
        """
    ...
    def recognize_actions(
        ...
        Args:
            detections: List of person detections (∫i˙PúnÍπ»)
            poses: List of person poses (∫iˇ‚nÍπ»)
            frame_shape: Frame shape (height, width) (’Ï¸‡b∂)
            
        Returns:
            List[ActionResult]: List of action recognition results (L’çXPúnÍπ»)
        """
        results = []
...
kL’çXPúíÔñ)
        ...
        Args:
            frame: Input frame (eõ’Ï¸‡)
            detections: Detection results (˙Pú)
            actions: Action recognition results (L’çXPú)
            
        Returns:
            np.ndarray: Frame with action visualizations (L’Ôñ’Ï¸‡)
        """
